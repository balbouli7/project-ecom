...
bookModel.deleteBook(id).then((result)=>{
    res.json({message:"supprime avec succye!!"})
    }).catch((err)=>{
        console.log(err)
        })
...
exports.PostgetmybooksUpdate = (req, res, next) => {
    ...
    const coverPath = req.files && req.files.cover && req.files.cover.length > 0
      ? `images/${req.files.cover[0].filename}`
      : req.body.oldCover;
  
    const livrePath = req.files && req.files.livre && req.files.livre.length > 0
      ? `images/${req.files.livre[0].filename}`
      : req.body.oldLivre;
  
    // Appel de la fonction de mise à jour avec l'ordre :
    // (bookId, userId, title, description, author, livre, cover)
